...
export default function YeniIlanPage() {
  ...
  const onSubmit = async (data: IlanForm) => {
    ...
    setYukleniyor(true)
    try {
      const yanit = await fetch('/api/ilanlar', {
        method: 'POST',
      ...
      if (!yanit.ok) {
        toast.error(json.hata || 'İlan oluşturulamadı.')
        return
      }

      toast.success('İlanınız başarıyla yayınlandı! 🎉')
      router.push(`/ilan/${json.ilan.id}`)
    } catch {
  ...
  if (status === 'unauthenticated') {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">İlan vermek için hesabınıza giriş yapın.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Yeni İlan Ver</h1>
        <p className="text-gray-500 text-sm mt-1">
          İlanınızı oluşturun, binlerce alıcıya ulaşın.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-5">
          <div className="flex items-start gap-3">
            <div className="bg-blue-400 p-2 rounded-xl shrink-0">
              <CheckCircle size={18} className="text-blue-900" />
            </div>
            <div>
              <p className="font-bold text-blue-900">İlanınıza Doping Alın!</p>
              <p className="text-blue-700 text-sm mt-0.5">
                İlanı yayınladıktan sonra vitrin, anasayfa sabitleme ve renkli yazı dopingleri
                alarak daha fazla kişiye ulaşın.
              </p>
              <Link
                href="/doping"
                className="inline-block mt-2 text-amber-800 font-bold text-sm hover:underline"
              >
                Doping paketlerini incele →
              </Link>
            </div>
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={yukleniyor}
          className="w-full flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-600 disabled:bg-gray-300 text-white font-bold py-4 rounded-2xl transition-colors text-base"
        >
          {yukleniyor ? (
          ...
          {yukleniyor ? 'İlan yayınlanıyor...' : 'İlanı Yayınla'}
        </button>
      </form>
    </div>
  )
